...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
library(magrittr)
kal_polygon <- sf::st_read("/Volumes/Macintosh HD - Data/Users/srirama/Documents/raviuitp/hospet_division/wgs_hospet_accessibility.shp") 
...
kal_routes <- read_sf("/Volumes/Macintosh HD - Data/Users/srirama/Documents/raviuitp/hospet_division/hospet_division.shp")
...
-   In total `r nrow(kal_routes)` trips were mapped for Hospete
...
```{r routesHos, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.cap = "Extent of services from Hospete"}
...
```{r accessHos, echo = FALSE, fig.cap = "Accessibility in Hospete"}
...
ggplot(kal_polygon ) + geom_sf(aes(fill = accessibility),col = "NA") +
  ...
  theme_bw()+
  geom_sf(data= city_sf %>% filter(Worksheet == "Hospet")) +
  geom_text(data= cities %>% filter(Worksheet == "Hospet"), aes(x= long, y = lat, label = Depot), vjust=-1)+
  theme(axis.title=element_blank())
