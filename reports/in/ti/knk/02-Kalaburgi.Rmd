...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
library(magrittr)
kal_polygon <- sf::st_read("/Volumes/Macintosh HD - Data/Users/srirama/Documents/raviuitp/kalburgi_division/wgs_kalburgi_accessibility.shp") 
...
kal_routes <- read_sf("/Volumes/Macintosh HD - Data/Users/srirama/Documents/raviuitp/kalburgi_division/kalburgi.shp")
...
-   In total `r nrow(kal_routes)` trips were mapped for Kalaburagi.
...
```{r routesKalas, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.cap = "Extent of services from Kalaburagi"}
ggplot() + 
...
```{r routesKala, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.cap = "Extent of services from Kalaburagi"}
# Get the rectangular bounding box
library(osmdata)
library(ggmap)
city_rect <- getbb(place_name = "Karnataka, India" )
mad_map <<- get_map(city_rect, maptype = "toner-background")
...
```{r accessKala, echo = FALSE, fig.cap = "Accessibility in Kalaburagi"}
