...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
library(magrittr)
kal_polygon <- sf::st_read("/Volumes/Macintosh HD - Data/Users/srirama/Documents/raviuitp/yadgiri_division/wgs_yadgiri_accessibility.shp") 
...
kal_routes <- read_sf("/Volumes/Macintosh HD - Data/Users/srirama/Documents/raviuitp/yadgiri_division/yadgiri_routes.shp")
...
-   In total `r nrow(kal_routes)` trips were mapped for Yadgiri
...
```{r routesYad, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.cap = "Extent of services from Yadgir"}
...
```{r accessYad, echo = FALSE, fig.cap = "Accessibility in Yadgiri"}
...
ggplot(kal_polygon ) + geom_sf(aes(fill = accessibility),col = "NA") +
  ...
  theme_bw()+
  geom_sf(data= city_sf %>% filter(Worksheet == "Yadgiri")) +
  geom_text(data= cities %>% filter(Worksheet == "Yadgiri"), aes(x= long, y = lat, label = Depot), vjust=-1)+
  theme(axis.title=element_blank())
